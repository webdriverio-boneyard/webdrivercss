...
var WebdriverCSS = function(webdriverInstance, options) {
    ...
    /**
     * Determine screenshot repository name & path to folder based on screenshotRoot
     */
    var seperators = this.screenshotRoot.split(path.sep).length - 1;

    this.repo = seperators >= 1 ? this.screenshotRoot.slice(this.screenshotRoot.lastIndexOf('/') + 1) : this.screenshotRoot;

    if (seperators >= 1 && (this.screenshotRoot[0] !== '.' || this.screenshotRoot[0] !== path.sep)) {
        this.pathToFolder = './' + this.screenshotRoot.slice(0, this.screenshotRoot.lastIndexOf('/') + 1);
    } else {
        this.pathToFolder = '.';
    }

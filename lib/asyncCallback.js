...
module.exports = function(err,res) {
    ...
    if (this.queuedShots.length > 0) {
         ...
         */
        if(this.screenWidth.length > 1) {
            this.screenWidth.shift();

            /**
             * if user passes specific screenWidth arguments for a single webdrivercss call
             * we have to decrease array elements of them as well to prevent endless loops
             */
            if(this.args[0].screenWidth) {
                this.args[0].screenWidth.shift();
            }
